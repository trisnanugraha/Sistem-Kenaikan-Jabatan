<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $("#tabelartikel").DataTable({
            ...
            "autoWidth": false,
            "language": {
                "sEmptyTable": "Data Artikel Masih Kosong"
            },
    ...
    function add() {
        ...
        $('.modal-title').text('Tambah Artikel Baru'); // Set Title to Bootstrap modal title
    }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.thumbnail == null) {
                    var thumbnail = "<?php echo base_url('assets/images/user/default.png') ?>";
                    $("#v_image").attr("src", thumbnail);
                } else {
                    var url = "<?php echo base_url('assets/images/') ?>";
                    $("#v_image").attr("src", url + data.thumbnail);
                }

                $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                $('.modal-title').text('Ubah Artikel'); // Set title to Bootstrap modal title
    ...
    function save() {
        ...
        formData = {
            id_artikel: $("#id").val(),
            judul_artikel: $("#judul").val(),
            thumbnail: $("#thumbnail").val().match(/[^\\/]*$/)[0],
            author: $("#author").val(),
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('artikel/insert') ?>";
        } else {
            url = "<?php echo site_url('artikel/update') ?>";
        }
            ...
            success: function(data) {
                ...
                if (data.status) //if success close modal and reload ajax table
                {
                    ...
                    if (save_method == 'add') {
                        Toast.fire({
                            icon: 'success',
                            title: 'Data Artikel Berhasil Disimpan!'
                        });
                    } else if (save_method == 'update') {
                        Toast.fire({
                            icon: 'success',
                            title: 'Data Artikel Berhasil Diubah!'
                        });
                    }
                } else {
